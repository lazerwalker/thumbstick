...
function assertThumb(test) {
    return function() {
        test.assertExists(
            "#thumb_" + collectionIndex + "_" + thumbIndex,
            "Thumb " +
                thumbIndex +
                " from collection " +
                collectionIndex +
...
casper.test.begin(
    "Thumbstick test page loads correctly",
    NB_TESTS,
    function suite(test) {
        var box;
        casper
             ...
             *
             \**********************************/
            // Assert we can create more than one thumbstick on dynamic
            .then(clickZone)
            ...
            // Assert we can't create more than one thumbstick on semi
            .then(clickZone)
            .then(assertThumb(test))
            .then(clickZone)
            .then(assertNotThumb(test))
            .then(function() {
                // Revert increment because last thumbstick doesn't exist
                thumbIndex -= 1;
            })
            /**********************************\
             *
